...
`ConsentGrant` (unique per user + client) records approved scopes. `/oauth/authorize` skips the consent screen and issues a code directly when a stored grant covers all requested scopes; broader scopes re-prompt and widen the grant on approval. The consent screen renders each requested scope as a checkbox (checked by default), so users can grant a subset: the granted set must be a subset of the request (a submitted superset is a 400 `invalid_scope`), unchecking everything is a denial, and scopes unchecked on the screen are removed from the stored grant via `upsert_grant(..., declined_scopes=...)` — so a declined scope re-prompts on the next authorize instead of silently returning. Scopes granted earlier but absent from the current request survive. `register_scope` also takes display metadata: `label` (short display name shown as the scope's heading, with `description` demoted to a summary line), `details` (full explanation behind a no-JS `<details>` disclosure), and `required_hint` (site-provided text rendered after the "Required" badge — Skrift itself only says "Required"). The consent page's colors all come from `--sk-color-*` theme variables, the permission list scrolls in-place with a scroll-gated Allow ("More ↓" until the bottom has been seen; plain Allow without JS), and sites can replace the whole template by overriding `oauth/authorize.html`. A scope registered with `register_scope(..., required=True)` is not user-declinable: the consent screen renders it locked (checked + disabled, value carried by a hidden field) and `authorize_post` re-asserts requested-and-required scopes server-side, so keeping only required scopes approves rather than denies — but a required scope the client never requested is never added. Grants cascade-delete when the client is deleted or pruned. Service: `skrift/db/services/oauth2_consent_service.py` (`find_grant`, `upsert_grant`, `revoke_grant`, `delete_grants_for_clients`).
